...
def _urls(query: str, location: str) -> list[str]:
    ...
    urls = [f"https://wellfound.com/jobs?keywords={q}"]
    if loc:
        urls.insert(0, f"https://wellfound.com/jobs?keywords={q}&location={loc}")
    return urls


async def search_wellfound(query: str, location: str = "", max_results: int = 20) -> list[dict]:
    return await search_public_job_board(
        ...
        blocked_markers=["sign up", "log in", "captcha"],
    )


async def search_jobs(candidate_profile: dict) -> list[dict]:
    return await search_wellfound(
